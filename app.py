...
if uploaded or use_fallback:
	if uploaded:
		...
		df_dataset = pd.read_excel( 'stores/excel/Combined Schedules.xlsx' )
...
with tabs[ 4 ]:
	...
	numeric_columns = [ c for c in df_original.columns 
	                    if df_original[ c ].dtype.kind in { 'i',  'f' } ]
...
with tabs[ 5 ]:
	...
	if st.button( '🚀 Train Classifier' ):
		try:
			...
			st.dataframe( df_classifier, use_container_width=True )
...
with tabs[ 6 ]:
	...
	if st.button( '🚀 Train Model' ):
		try:
			...
			st.dataframe( df_regressor, use_container_width=True )
